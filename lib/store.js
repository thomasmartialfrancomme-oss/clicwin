...
function credit(user, amount, type, note, dec) {
  const k = Math.pow(10, (dec == null ? 2 : dec));
  user.balance = Math.round((user.balance + amount) * k) / k;
  if (amount > 0) user.totalEarned = Math.round((user.totalEarned + amount) * k) / k;
...
// ------------- Robinet (faucet) -------------
// État courant du robinet pour un membre : temps d'attente restant,
// nombre de réclamations déjà faites aujourd'hui et plafond.
function faucetState(userId) {
  const f = config.faucet || {};
  const u = findUserById(userId);
  if (!u) return null;
  const now = Date.now();
  const last = u.faucetAt || 0;
  const interval = (f.intervalSec || 120) * 1000;
  let wait = Math.ceil((last + interval - now) / 1000);
  if (wait < 0) wait = 0;
  const today = dateStr().slice(0, 10);
  let claimed = u.faucetCount || 0;
  if (u.faucetDate !== today) claimed = 0;
  return {
    enabled: !!f.enabled,
    intervalSec: f.intervalSec || 120,
    claimed,
    cap: f.dailyCap || 100,
    wait,
    nextAt: last + interval
  };
}

// Réclame le gain du robinet. Retourne {ok, amount, balance, claimed, cap, wait}.
function faucetClaim(userId) {
  const f = config.faucet || {};
  const u = findUserById(userId);
  if (!u) return { ok: false, code: 'auth' };
  if (!u.active) return { ok: false, code: 'blocked' };
  if (!f.enabled) return { ok: false, code: 'off' };

  const now = Date.now();
  const interval = (f.intervalSec || 120) * 1000;
  const last = u.faucetAt || 0;
  const today = dateStr().slice(0, 10);
  if (u.faucetDate !== today) { u.faucetDate = today; u.faucetCount = 0; }

  const cap = f.dailyCap || 100;
  let wait = Math.ceil((last + interval - now) / 1000);
  if (wait < 0) wait = 0;
  if (wait > 0) return { ok: false, code: 'wait', wait, claimed: u.faucetCount, cap };
  if (u.faucetCount >= cap) return { ok: false, code: 'cap', claimed: u.faucetCount, cap };

  // gain aléatoire entre min et max (arrondi 4 décimales pour éviter la dérive des floats)
  let amount = (f.min || 0.0003) + Math.random() * Math.max(0, (f.max || 0.0007) - (f.min || 0.0003));
  amount = Math.round(amount * 10000) / 10000;
  if (amount <= 0) amount = f.min || 0.0003;

  u.faucetAt = now;
  u.faucetCount += 1;
  // Pas de commission parrain ni d'activation de parrainage sur le robinet :
  // sinon des multi-comptes exploitent le système pour activer des filleuls gratuitement.
  credit(u, amount, 'faucet', 'faucet', 4);
  return { ok: true, amount, balance: u.balance, claimed: u.faucetCount, cap, wait: f.intervalSec || 120 };
}
