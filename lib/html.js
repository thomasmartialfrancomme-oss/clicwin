...
function money(n, lang) {
  ...
  // Les montants très petits (robinet) s'affichent avec 4 décimales, les autres avec 2.
  const s = Math.abs(n) < 0.01
    ? (Math.round(n * 10000) / 10000).toFixed(4)
...
function layout({ lang, user, active, title, content, extraHead, bodyScripts }) {
  ...
  const pages = [
    ...
    { id: 'faucet', href: '/faucet', label: t_('faucet'), need: true },
...
<header class="top">
  ...
  <div class="mnav">
    <div class="wrap">
      ...
      ${user ? `<a href="/faucet">${esc(t_('faucet'))}</a><a href="/offers">${esc(t_('offers'))}</a><a href="/withdraw">${esc(t_('withdraw'))}</a><a href="/referrals">${esc(t_('referrals'))}</a>` : ''}
...
function balanceCard(lang, user) {
  ...
  const fc = user.faucetDate === today ? (user.faucetCount || 0) : 0;
  return `<section class="card hero-bal">
    <div>
      ...
      <div class="muted small">${esc(t_('faucetToday'))}: <b>${fc}</b></div>
    </div>
    <div class="hero-actions">
      <a class="btn primary" href="/faucet">${esc(t_('goFaucet'))}</a>
